...
if pm2 list | grep -q "kurye-backend.*online"; then
    ...
    echo -e "${BLUE}🔗 Health check: http://red.enucuzal.com:$PORT/health${NC}"
    echo -e "${BLUE}👨‍💼 Admin panel: http://red.enucuzal.com:$PORT/admin${NC}"
    echo -e "${BLUE}📷 Image upload: http://red.enucuzal.com:$PORT/api/uploadImage${NC}"
    echo ""
